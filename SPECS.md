...
* Opt-in automatic heap snapshots detect sustained 30-minute growth only when both RSS and JS heap pass configured thresholds and the heap share is high enough; `memory.autoSnapshot.*` enforces a 30-minute cooldown, per-run capture cap, and retained automatic-snapshot cap under `YASH_DATA_DIR/logs/heap-snapshots/`
    ...
    * Command `/action [action-id] [key=value ...]` - lists public IPC-safe actions, invokes actions directly when all args are optional, and shows help/examples when required args still need values
        ...
        * The live TUI may also invoke visible non-IPC actions through `/action`; modal-only actions such as `obs.shutdown.config.tui`, `obs.startup.config.tui`, and `obs.source-recaller.config.tui` remain blocked over IPC (`bun run cmd`)
    * Command `/scripts | /scripts list | /scripts install <example-id> [repair|force] [copy|link]` - list bundled example scripts and install one into `YASH_DATA_DIR/scripts/<example-id>` without overwriting existing files unless repair/force is requested explicitly
        ...
        * Bundled examples currently include `obs-scene-change`, `obs-startup`, `obs-source-recaller`, `obs-audio-routing`, and `obs-alerts`
        ...
        * Loader-backed user scripts and bundled example scripts must export `scriptDefinition`, own `YASH_DATA_DIR/scripts/<scriptId>/config.jsonc`, and let YASH inject `/action <prefix>.config`, `/action <prefix>.config.tui`, `/action <prefix>.config.open`, and `/action <prefix>.actions`
        ...
        * `<prefix>.config.tui` must edit the same `config.jsonc` surface from the live TUI and stay blocked over IPC
        * `<prefix>.config.open` must open that same `config.jsonc` path in `$EDITOR`, using `$TERMINAL` when the editor is terminal-based
        * `<prefix>.actions` must open a TUI modal that lists the script's visible actions and helps invoke or prefill them
        * Script code must not register framework-owned ids ending in `.config`, `.config.tui`, `.config.open`, or `.actions`
        * Bundled `obs-scene-change` actions: framework-owned `config`, `config.tui`, `config.open`, `actions`, plus behavior action `activate`; `activate` switches OBS to `scene=` when passed or to the configured `defaultScene` otherwise, so voice bridges can trigger scene changes without needing custom app code
        ...
        * The generic script config modal must provide clear visual hierarchy for nested config: a status summary for root `enabled`, a legend for object/array/item markers, whitespace indentation for nesting, compact suffix markers like `sectionName {}` / `sectionName []`, and stronger separation between top-level sections
        ...
        * Bundled `obs-startup` actions: framework-owned `config`, `config.tui`, `config.open`, `actions`, plus behavior actions `begin`, `live`, `cancel`, and `status`; `live` jumps straight to the configured `liveScene` and reapplies the go-live show/unmute/chat steps without running the prepare/countdown phases; all persisted startup settings now live in `YASH_DATA_DIR/scripts/obs-startup/config.jsonc`
        * Bundled `obs-shutdown` exposes framework-owned `/action obs.shutdown.config`, `/action obs.shutdown.config.tui`, `/action obs.shutdown.config.open`, and `/action obs.shutdown.actions` against `YASH_DATA_DIR/scripts/obs-shutdown/config.jsonc`, while `initiate`, `cancel`, and `status` remain the behavior actions
        ...
        * `obs-source-recaller` actions: framework-owned `config [startPaused=<true|false>]`, `config.tui`, `config.open`, `actions`, plus behavior actions `save source=<source|scene.source> [stage=<inputSettings|sceneItemTransform|sceneItemEnabled>]`, `load source=<source|scene.source>`, `list`, `explore [scene=<scene>]`, `pause`, and `resume`
        * `obs-audio-routing` actions: framework-owned `config`, `config.tui`, `config.open`, `actions`, plus behavior actions `status`, `wiring [wait=<duration>]`, `candidates`, `search query=<text>`, `addStreamTarget`, `addMusicTarget`, `restoreDefaultExclusions`, and `repairDefaultExclusions`; persisted rules live in `YASH_DATA_DIR/scripts/obs-audio-routing/config.jsonc`, while runtime candidates remain in-memory only for the current session
        * `obs-audio-routing.addStreamTarget` and `obs-audio-routing.addMusicTarget` add target rules from `/action` without hand-editing JSON; `processBinary=` is required, optional args include `id=`, `childProcessBinary=`, `applicationName=`, `mediaName=`, `sourceSinkName=`, `windowClass=`, `windowTitleRegex=`, `enabled=`, `replace=`, and `notes=`, with autocomplete for arg names and common `sourceSinkName` values such as `easyeffects_sink`
        * `obs-audio-routing` keeps tracked previous sinks for streams it moved itself and restores those streams when the script is disabled, but only if the stream is still on the script-owned sink so manual user moves are respected
        * `obs-audio-routing` config also includes `obsStreaming.enableOnStreamStart`, `obsStreaming.disableOnStreamStop`, `obsStreaming.enableOnObsConnect`, and `obsStreaming.disableOnObsDisconnect` so OBS stream-state or OBS connection-state events can persistently enable or disable the routing script automatically
        * `obs-audio-routing` config may declare `routing.linkWhenSourceSinkMatches[]` for intercepting/processing sinks (for example `easyeffects_sink`); when a matched stream currently lives on one of those sinks, YASH must keep that sink in place and add/remove PipeWire links to `Music` / `Stream` instead of calling `pactl move-sink-input`; route matching for process names and intercepting sink names tolerates separator/case differences such as `Google Chrome` vs `google-chrome` and `EasyEffects Sink` vs `easyeffects_sink`; pre-existing links must be treated as a quiet no-op and must not spam repeated `already linked` chat/event messages
        * `obs-audio-routing.wiring` can accept `wait=500ms` or `wait=5s` so the user can switch focus after sending the command before the focused-window/audio snapshot is taken
        * `obs-alerts` actions: framework-owned `config`, `config.tui`, `config.open`, `actions`, plus behavior actions `list`, `add`, `remove`, `enable`, `pause`, `resume`, `status`, and `test`; persisted rules live in `YASH_DATA_DIR/scripts/obs-alerts/config.jsonc`
        * `obs-alerts` listens for live Twitch/Kick/YouTube activity events after the script loads and matches enabled rules by `platform=` (`twitch | kick | youtube`) and comma-separated `types=`; matching rules set `textSource` (or legacy `source`) to `textTemplate` (default `{user}`) and set `showSource` (or legacy `source`) visible, leaving auto-hide to OBS filters unless the user adds their own OBS-side behavior
        * `obs-alerts.add` supports autocomplete for `platform=` enum values, `types=` activity values, `scene=` via `obs.scenes`, and `source=` / `textSource=` / `showSource=` via `obs.sceneSources` scoped to `scene=`
        * `obs-audio-routing.restoreDefaultExclusions` re-adds only missing shipped default exclusions, while `obs-audio-routing.repairDefaultExclusions` also repairs malformed shipped defaults by reserved id without touching user-created exclusions
        * `config.open` actions open `YASH_DATA_DIR/scripts/<scriptId>/config.jsonc` in `$EDITOR`, using `$TERMINAL` when the editor is terminal-based
...
* Includes `memory.autoSnapshot.enabled`, `memory.autoSnapshot.minRssGrowthMb`, `memory.autoSnapshot.minHeapGrowthMb`, `memory.autoSnapshot.minHeapSharePercent`, `memory.autoSnapshot.cooldownMinutes`, `memory.autoSnapshot.maxPerRun`, and `memory.autoSnapshot.maxRetained` for bounded automatic heap snapshots
    ...
    * `/stream` modal templates: includes a template-name field plus filtered saved-name suggestions; `F5` saves the current modal draft plus selected platforms into `streamTemplates.items[<name>]` and persists that name as `streamTemplates.activeName`; `F8` restores the template currently named in the field, including provider selection and category/search fields, and refreshes the visible YouTube category selector immediately even when focus is elsewhere; `F10` deletes the template currently named in the field and keeps the remaining set persisted
    ...
    * Chatter info modal: left-click any chat message, or press Enter while in browse mode (Escape from an empty message box enters browse mode; ↑/↓ selects a message; Escape exits browse mode), to open a modal showing:
...
- Web UI page routes (`/`, `/unified`, `/sidebyside`) must stay on Bun's direct HTML import model from `src/index.ts` (`import indexHtml from '../index.html'`, etc.) and be registered directly in `Bun.serve({ routes })`.
- Do not add a custom runtime HTML/CSS/JS build-cache layer for the Web UI unless a task explicitly requires it; Bun owns the page asset graph for the normal architecture.
...
- When text-only tmux capture is insufficient for TUI refactoring/fix proof, use the `ghostty-grim-screenshot` skill to capture the focused Ghostty/tmux window through Hyprland `hyprctl` + `grim` into `[tmp]/screenshots/`
