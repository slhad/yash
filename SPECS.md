...
- Usable TUI
    ...
    * Command `/scripts | /scripts list | /scripts install <example-id> [repair|force] [copy|link]` - list bundled example scripts and install one into `YASH_DATA_DIR/scripts/<example-id>` without overwriting existing files unless repair/force is requested explicitly
        ...
        * `obs-audio-routing` actions: `config`, `configTUI`, `status`, `wiring [wait=<duration>]`, `candidates`, `search query=<text>`, `restoreDefaultExclusions`, and `repairDefaultExclusions`; persisted rules live in `YASH_DATA_DIR/scripts/obs-audio-routing/config.jsonc`, while runtime candidates remain in-memory only for the current session
